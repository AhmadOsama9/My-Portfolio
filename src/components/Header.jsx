...
const NavBar = React.memo(() => {
  ...
  return (
    <nav
      className="fixed top-0 left-0 w-full p-2 py-4 backdrop-blur-lg bg-neutral-900/80 shadow-lg z-50 border-b border-primary-500/20"
    >
      <div className="flex justify-between items-center max-w-7xl mx-auto px-4">
        ...
        >
          {isOpen ? (
            <X className="w-6 h-6 text-primary-400 stroke-current" strokeWidth={2} />
          ) : (
            <Menu className="w-6 h-6 text-primary-400 stroke-current" strokeWidth={2} />
          )}
        </motion.button>
      </div>
